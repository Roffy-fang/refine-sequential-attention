...
function LSTM_Armour.lstm(input_size1, input_size2, output_size, rnn_size, attSize, n, dropout, dropout_t, active) 
  ...
  for L = 1,n,2 do
    ...
    local prev_ha = inputs[L*2+2]
    local prev_ca = inputs[L*2+1]
    local prev_h = inputs[L*2+4]
    local prev_c = inputs[L*2+3]
    if dropout_t > 0 then prev_ha = nn.Dropout(dropout_t)(prev_ha):annotate{name='drop_' .. L} end -- apply dropout, if any
    if dropout_t > 0 then prev_h = nn.Dropout(dropout_t)(prev_h):annotate{name='drop_' .. L} end -- apply dropout, if any
    -- the input to this layer
    if L == 1 then 
      local hh = nn.JoinTable(2)({prev_h,prev_ha})
     	local alpha = LSTM_Armour.Attention_Unit(rnn_size*2, input_size2, attSize)({hh, imgs})   
    -- soft attention, glimpse
    ...
    -- soft attention, glimpse
	     x1 = LSTM_Armour.makeWeightedSumUnit()({imgs, alpha}) 
       x2 = outputs[(L-1)*2]  -- lower layer output: next_h
    ...
    local i2h = nn.Linear(input_size_L1, 4 * rnn_size)(x1):annotate{name='w2h_'..L} -- word embedding input 
    -- to avoid double bias terms
   -- local v2h = nn.Linear(input_size_L2, 4 * rnn_size, false)(x2):annotate{name='v2h_'..L} -- visual input 
	
    local h2ha = nn.Linear(rnn_size, 4 * rnn_size, false)(prev_ha):annotate{name='h2h_'..L}
    local all_input_sumsa =  nn.CAddTable()({i2h, h2ha})
    if  L > 2 and active == 1 then
      local at = outputs[(L-2)*2]
      if dropout > 0 then at = nn.Dropout(dropout)(at):annotate{name='drop_' .. L} end -- apply dropout, if any(may use different dropout rate in the futures)
      local v2ha = nn.Linear(rnn_size, 4 * rnn_size, false)(at):annotate{name='v2h_'..L} -- visual input 
       all_input_sumsa =  nn.CAddTable()({all_input_sumsa, v2ha})
    end
    local reshapeda = nn.Reshape(4, rnn_size)(all_input_sumsa)
    ...
    local n1a, n2a, n3a, n4a = nn.SplitTable(2)(reshapeda):split(4)
    -- decode the gates
    local in_gatea = nn.Sigmoid()(n1a)
    local forget_gatea = nn.Sigmoid()(n2a)
    local out_gatea = nn.Sigmoid()(n3a)
    -- decode the write inputs
    local in_transforma = nn.Tanh()(n4a)
    -- perform the LSTM update
    local next_ca           = nn.CAddTable()({
        nn.CMulTable()({forget_gatea, prev_ca}),
        nn.CMulTable()({in_gatea,     in_transforma})
      })
    -- gated cells form the output
    local next_ha = nn.CMulTable()({out_gatea, nn.Tanh()(next_ca)})
    local i2hi = nn.Linear(input_size_L2, 4 * rnn_size, false)(next_ha):annotate{name='v2h_'..L} -- visual input 
    -- next is 'current', which will be used as input at the next timestep
    table.insert(outputs, next_ca)
    table.insert(outputs, next_ha)
   
    local v2h = nn.Linear(input_size_L2, 4 * rnn_size, false)(x2):annotate{name='v2h_'..L} -- visual input 
    local h2h = nn.Linear(rnn_size, 4 * rnn_size, false)(prev_h):annotate{name='h2h_'..L}
    local all_input_sums =  nn.CAddTable()({i2hi, v2h, h2h})
